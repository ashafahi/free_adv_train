...
# Setting up training parameters
max_num_training_steps = config.train_steps
num_output_steps = config.out_steps
num_summary_steps = config.summary_steps
num_checkpoint_steps = config.checkpoint_steps
...
replay_m = config.replay_m
model_dir = config.model_dir + '_m%d_eps%.1f_b%d'%(replay_m,epsilon,batch_size)
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
  ...
  for ii in range(max_num_training_steps):
    ...
    if ii % num_summary_steps == 0:
      ...
      print('    training nat accuracy {:.4}% -- validation nat accuracy {:.4}%'.format(train_acc * 100, val_acc*100))
      sys.stdout.flush()
